...
/*
 * There are two types of object is the world: those is the database
 * and those that are not. Objects in the database should be allocated
 * with dbm_malloc and friends. Those that are not should be allocated
 * using malloc and friends. The way to tell which pool of memory
 * the object's data should be allocated from is to look at the node
 * containing the definition for the object. If the DNF_CONFIG flag
 * is set, then the object should be allocated from the database.
 */
#ifndef DDL_USE_PRIVATE_MALLOC
#define DDL_PRIVATE_MALLOC malloc
#define DDL_PRIVATE_CALLOC calloc
#define DDL_PRIVATE_FREE free
#endif

ddl_boolean_t ddl_is_dbm(dbm_memory_t *dbmp, ddl_node_t *dnp);
void *ddl_malloc(dbm_memory_t *dbmp, ddl_node_t *dnp, size_t size);
void *ddl_calloc(dbm_memory_t *dbmp, ddl_node_t *dnp, size_t count,
		 size_t size);

void db_free_oid (dbm_memory_t *dbmp, ddl_object_t *dop);
void ddl_free(dbm_memory_t *dbmp, ddl_node_t *dnp, void *ptr);
