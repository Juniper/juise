 ...
 *
 *
 * The allocated memory is split up in pages (PAGE_SIZE).
 *
 *
 * Memory chunks that are > PAGE_SIZE / 4 occupy at least
 ...
 *
 *
 * Memory chunks that are <= PAGE_SIZE / 4 share a page
...
get_frag_offset_p (dbm_memory_t *dbmp, u_int32_t size, u_int32_t *recorded_size_p)
{
	...
	frag_offset_p = NULL;
	recorded_size = (size + PAGE_SIZE - 1) & ~PAGE_MASK;
    }
...
alloc_pages (dbm_memory_t *dbmp, u_int32_t page_count)
{
	...
	free_p = (dbm_free_page_t *)((caddr_t)dbmp + free_page);

	if (free_p->dfp_next != DBM_OFFSET_NULL) {
	    next_free_p = (dbm_free_page_t *)((caddr_t)dbmp + free_p->dfp_next);
	...
	if (page_count_probe > page_count) {
	    ...
	    free_page_next = free_page + page_count * PAGE_SIZE;
	    page_count_left = page_count_probe - page_count;

	    free_p = (dbm_free_page_t *)((caddr_t)dbmp + free_page_next);
    ...
    if ((dbmp->dm_top + PAGE_SIZE - 1) / PAGE_SIZE + page_count > DBM_MEMORY_PAGE_COUNT)
	return DBM_OFFSET_NULL;

    new_chunk = dbm_mmap(dbmp, page_count * PAGE_SIZE);
    if (new_chunk) {
	free_page = (caddr_t)new_chunk - (caddr_t)dbmp;

	assert(!(free_page & PAGE_MASK));
	assert(free_page / PAGE_SIZE + page_count <= DBM_MEMORY_PAGE_COUNT);
...
free_pages (dbm_memory_t *dbmp, dbm_offset_t start, u_int32_t page_count)
{
    ...
    free_p = (dbm_free_page_t *)((caddr_t)dbmp + start);
    free_p->dfp_prev = DBM_OFFSET_NULL;
...
dbm_malloc (dbm_memory_t *dbmp, size_t size)
{
	...
	if (*frag_offset_p != DBM_OFFSET_NULL) {
	    header_p = (dbm_page_header_t *)((caddr_t)dbmp + (*frag_offset_p & ~PAGE_MASK));
	    slot_idx = ((*frag_offset_p & PAGE_MASK) - sizeof(dbm_page_header_t)) / recorded_size;
	    header_p->dph_usage[slot_idx / (sizeof(dbm_mask_t) * NBBY)] |=
		1 << (slot_idx % (sizeof(dbm_mask_t) * NBBY));

	    free_p = (dbm_free_slot_t *)((caddr_t)dbmp + *frag_offset_p);

	    if (free_p->dfs_next != DBM_OFFSET_NULL) {
		next_free_p = (dbm_free_slot_t *)((caddr_t)dbmp + free_p->dfs_next);
    ...
    size_info_p = dbmp->dm_size_info + new_pages / PAGE_SIZE;
    while (page_count > 0)
	size_info_p[--page_count] = recorded_size;

    if (frag_offset_p) {
	header_p = (dbm_page_header_t *)((caddr_t)dbmp + new_pages);
	...
	*frag_offset_p = new_pages + offset;
	...
	for (;;) {
	    free_p = (dbm_free_slot_t *)((caddr_t)dbmp + new_pages + offset);
	    offset += recorded_size;
	    free_p->dfs_prev = prev_offset;
	    if (offset + recorded_size > PAGE_SIZE) {
...
dbm_free (dbm_memory_t *dbmp, void *ptr)
{
    ...
    page_offset = slot_offset & ~PAGE_MASK;
    size_info_p = dbmp->dm_size_info + slot_offset / PAGE_SIZE;

    frag_offset_p = get_frag_offset_p(dbmp, *size_info_p, NULL);

    if (frag_offset_p) {
	header_p = (dbm_page_header_t *)((caddr_t)dbmp + page_offset);
	slot_idx = ((slot_offset & PAGE_MASK) - sizeof(dbm_page_header_t)) / *size_info_p;
	...
	*usage_p &= ~mask;
	...
	free_p = (dbm_free_slot_t *)((caddr_t)dbmp + slot_offset);
	...
	*frag_offset_p = slot_offset;
	...
	for (offset = sizeof(dbm_page_header_t);
	     offset + *size_info_p <= PAGE_SIZE;
	     offset += *size_info_p) {
	    free_p = (dbm_free_slot_t *)((caddr_t)dbmp + page_offset + offset);

	    if (free_p->dfs_prev != DBM_OFFSET_NULL) {
		prev_free_p = (dbm_free_slot_t *)((caddr_t)dbmp + free_p->dfs_prev);
	...
	page_count = (*size_info_p + PAGE_SIZE - 1) / PAGE_SIZE;
...
dbm_realloc (dbm_memory_t *dbmp, void *ptr, size_t size)
{
    ...
    page = ((caddr_t)ptr - (caddr_t)dbmp) / PAGE_SIZE;
...
main(int argc, char **argv)
{
    ...
    dbmp->dm_top = (sizeof(*dbmp) + PAGE_SIZE - 1) & ~PAGE_MASK;
